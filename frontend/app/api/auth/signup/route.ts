...
export async function POST(req: Request) {
  try {
    ...
    if (!r1.ok) {
      // 409 などをそのまま返す
      return new Response(await r1.text(), { status: r1.status, headers: { "Content-Type": r1.headers.get("Content-Type") ?? "application/json" } });
    }

    // 2) 直後に Spring でログイン → token 取得
    ...
    if (!r2.ok) return Response.json({ error: "auto login failed" }, { status: 502 });
